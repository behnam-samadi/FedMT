...
def split_dataset_by_labels(dataset, n_classes, n_clients, n_clusters, alpha, frac, seed=1234):
    ...
    client_shares = np.zeros((n_clients , n_classes))
    for i in range(n_clients):
      for j in clients_indices[i]:
        _, label = dataset[j]
        client_shares[i][label] += 1

    np.save("clients_indices.npy", clients_indices)
    np.save("clients_shares.npy", client_shares)
